...
**D39 第 4 轮（codex 二轮复核 G1 至 G4 全收）**：G1（重要，F3 的回归）`[Console]::OpenStandardInput()` 在 PS 5.1 重定向下从流末尾起读恒空（宿主启动时已把 stdin 预读进管道输入集合），实测三 harness 全空解析；终版读法 = **`$input` 惰性枚举 join 还原**，且读前先 `[Console]::InputEncoding = UTF8`（先设后读，中文不乱码），管道 / 重定向 / WSL interop 三态实测全过。追因发现更深的第三层根因：**ps1 文件必须带 UTF-8 BOM 落盘**（Windows PowerShell 5.1 对无 BOM 的 .ps1 按系统 ANSI 码页解析，脚本内中文注释乱码破坏语句，`$input` 读法被殃及；纯 ASCII 探针正常、真 shim 空解析的现场即此），deploy 侧 ps1 落盘加 BOM，state 输出文件仍无 BOM（serde 不吃 BOM）。G2（中，验证盲区）verify 的 parse_state 只看 state 四态，event 空（shim 空解析）也绿：判据补「event 非空」，并新增集成钉 `deployed_state_shim_roundtrips_stdin_payload`（直管道喂落盘载体 shim，断言 event/state/session 与输入一致，含中文载荷；event 大小写不敏感比对，因 guard 委托的 `hst hook status` 也会写 state 并规范化事件名小写）。G3（小）ps1 Notification 匹配去 `(?i)` 与 sh 版同集（大小写敏感）。G4（小）is_ours 补外来前缀族负例断言（hst-logger / oma-tool）。
