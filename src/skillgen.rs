...
//! hst；旧牌 ohmyagents 同体双写兼容窗至 1.2.0，翻案 D14/D22/D45 旧牌
//! 保留裁定）。
...
/// D49 兼容窗旧牌（至 1.2.0 摘双写加清扫）。
pub const SKILL_NAME_LEGACY: &str = "ohmyagents";
/// D49 兼容窗 CTA（旧牌 description 首带，引导改用 hst）。
pub const SKILL_LEGACY_CTA: &str = "旧牌兼容窗至 1.2.0，建议改用 hst 技能。";

/// 渲染完整 SKILL.md（canonical，name = hst）。
pub fn render_skill(root: &ClapCommand) -> String {
    render_skill_as(root, SKILL_NAME, "")
}

/// D49 兼容窗：旧牌同体双写（name = ohmyagents，description 首带 CTA）。
pub fn render_skill_legacy(root: &ClapCommand) -> String {
    render_skill_as(root, SKILL_NAME_LEGACY, SKILL_LEGACY_CTA)
}

fn render_skill_as(root: &ClapCommand, name: &str, cta: &str) -> String {
...
mod tests {
    ...
    fn renders_frontmatter_and_adaptive_rows() {
        ...
        // D49：canonical 技能名翻 hst（翻案 D14/D22/D45 旧牌保留裁定）。
        assert!(md.starts_with("---\nname: hst\ndescription: hst "));
        // D49 兼容窗：旧牌同体双写，name 加 CTA 各对各的。
        let legacy = render_skill_legacy(&tree);
        assert!(legacy.starts_with("---\nname: ohmyagents\ndescription: 旧牌兼容窗至 1.2.0"));
        assert!(legacy.contains("# hst 命令速查"));
        assert!(md.contains(
