...
fn skill_md() -> String {
    skill_md_as("hst", "hst 部署配置命令图：init、诊断、hook、状态栏、trace")
}

/// D49 兼容窗：旧牌 ohmyagents 同体双写（description 首带 CTA），1.2.0 摘。
fn skill_md_legacy() -> String {
    skill_md_as(
        "ohmyagents",
        "旧牌兼容窗至 1.2.0，建议改用 hst 技能。hst 部署配置命令图：init、诊断、hook、状态栏、trace",
    )
}

fn skill_md_as(name: &str, description: &str) -> String {
...
/// D49：体由调用方传入（canonical hst 或兼容窗旧牌），判等与覆写各对各的体。
...
fn deploy_skills(root: &Path, report: &mut DeployReport) -> Result<(), String> {
    ...
    let legacy_source = root.join(".agents").join("skills").join("ohmyagents");
    write_skill(&legacy_source.join("SKILL.md"), &skill_md_legacy(), report)?;
    for target in [".claude", ".grok", ".kimi-code"] {
        ...
        write_skill(
            &base.join("ohmyagents").join("SKILL.md"),
            &skill_md_legacy(),
            report,
        )?;
    }
...
fn deploy_kimi_project(root: &Path, report: &mut DeployReport) -> Result<(), String> {
    // D49：canonical hst（兼容窗 ohmyagents 由 deploy_skills 双写覆盖）。
...
mod tests {
    ...
    fn retire_strips_project_registrations_and_shims() {
        ...
        assert!(shims.is_dir());
        // skills 仍项目级部署。D49：canonical hst 加旧牌 ohmyagents 兼容窗双写。
        assert!(root
            .join(".agents")
            .join("skills")
            .join("hst")
        ...
        .unwrap();
        assert!(compat.contains("name: ohmyagents"));
        assert!(compat.contains("旧牌兼容窗至 1.2.0"));
