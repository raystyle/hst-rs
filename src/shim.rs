...
$agent = "$($args[0])"
if (-not $agent) { exit 0 }
# 字节态读 stdin（codex review F3）：[Console]::In 走控制台输入编码，中文
# session 与路径会乱码；StreamReader 钉 UTF-8 无 BOM 解码。
...
if ($event -eq 'Notification' -and $raw -match '(?i)permission') { $state = 'blocked' }
