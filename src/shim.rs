...
/// 掉 `/c`——注册串改 `powershell -NoProfile -ExecutionPolicy Bypass -File
...
if ($event -eq 'Notification' -and $null -ne $j) {
  $n = [string]$j.notification_type
  if (-not $n) { $n = [string]$j.notificationType }
  if (-not $n) { $n = [string]$j.matcher }
  if (-not $n) { $n = [string]$j.message }
  if ($n -like '*permission*') { $state = 'blocked' }
}
...
pub fn deploy_shims_with(
    ...
    // 并存（跨 OS 共享项目同 D28 语义）。
    let ps1 = dir.join("hst-state.ps1");
    if write_if_changed(&ps1, STATE_PS1)? {
        wrote.push(ps1);
    }
