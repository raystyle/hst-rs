...
D49 技能名翻 hst 加旧名兼容窗（2026-09-14 立项，用户裁「走」翻案旧牌保留）：
...
- [ ] codex 评审对齐后推 main（封版时点另裁）；1.2.0 摘兼容双写加清扫已入队列
...
- [ ] 1.2.0 摘 ohmyagents 技能兼容双写加清扫（D49 兼容窗终点：用户级与项目级旧牌目录、skillgen legacy 渲染与 deploy 双写面一并退役，沿 D29 至 D45 模式）
