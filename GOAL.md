...
> 迁移注记（2026-09-15）：目标轨迹职责由 AGENTS 头部定位加 REQ 承接；本文件为历史档案，时间线留档不再更新。
...
- **日期**：2026-09-15。
- **起点**：用户令「hst 的状态栏在第一行加年月日现在时间」立项 D51，第 2 轮实弹观感裁「时间显示到分钟 不要秒」（分钟粒度下渲染事件重绘即活钟）；随后封版令 v1.2.0（D48 至 D51 攒批）。

D50 前起点（2026-09-15，S029 追记（bypass 残余阻塞分类学）hst 落点评估节候选转正，用户令「开工」立项 D50：doctor yolo 诊断面补三项可诊断信号（项目层 defaultMode 非 bypass 遮蔽、项目层 ask 规则、读块键）；随批修既有 yolo 检查两缺口（漏 settings.local.json 层、项目层 bypass-only 假阳性，2.1.257 起项目层 bypass 被忽略）。
...
- **锚定的目标**：无（D51 已交付并随 v1.2.0 封版，2026-09-15 当日闭环；队列遗项 D47/D48 F3 测试钉与 G2 grok 闸门）。
...
| 日期 | 进展 |
| --- | --- |
| 2026-09-15 | **D53 交付（init 并入状态栏面）**：deploy_all 尾部并入 deploy_script 加四家 merge（home 形参 hst 根，第一版传错被测试抓出）；deploy_script 与 grok cmd 加 claude 与 codex 写入全面内容判等幂等；codex F1（merge 显式 user_home 透传修测试缝泄漏，测试改真机配置的暗病连根拔）F2（claude 与 codex 写入判等加四配置 mtime 断言）F3（面扩张舰队知会）；verify live 搅真机 mtime 系 D28 既有设计非回归；185 单测加 28 集成绿；d04ed1c 加 82fa3e0 推 main CI 绿，舰队回执已发 |
...
| 2026-09-14 | **D47 交付**：codex 四发现全收（F1 S028 追记、F2 INDEX 标签、F4 README 翻名单列、F3 取参纯函数重构入队）加 ohmycloud 对线无异议后推 main（6995cb3 加 11fffa5）；CI run 34840283806 六岗全绿，dev release 六资产 windows 位出 `hst-x86_64-pc-windows-gnu.zip`（msvc 退场）、镜像 hst/dev 同步到货；未动版本号未推 tag，herdr 回执 ohmycloud |
...
| 2026-09-13 | **D42 收口加 v1.1.1 发版**：codex 三轮评审（F1 升级破面 = 默认行去重、F2 编号撞号改 D42、F3 两处口径、H1 老配置原样升级语义 + 边角收紧非空老行）零异议；tag v1.1.1 打在 3cfd31a，CI 绿、镜像双段到货、win digest 三方对账全等（edfd970c8635）、herdr 知会 ohmycloud |
| 2026-09-13 | **D42 状态栏三行重分组**（用户裁定双排版嫌丑）：三行定名项目状态 / agent 状态 / 运行时状态（用户补名）；`segments3` 第三行键加拼装器泛化（任意行数）加机读标记放宽为任一行含（agent 态迁二行，S025 契约随批）；168 单测加 28 集成绿（kimi 并一行同钉收 G2），本机 dogfood 三行实弹；目标随批发 v1.1.1 |
...
- 当前目标：无（D47 归档于锚点时间线，2026-09-14 当日闭环：评审对齐推 main、CI 六岗全绿、dev 源与镜像出 gnu 资产；遗项在 TODO 队列）。
